...
function CapsuleSkeleton() {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-pulse">
      <div className="lg:col-span-8 bg-surface-container-low rounded-3xl h-80 border border-outline-variant/10" />
      <div className="lg:col-span-4 flex flex-col gap-4">
        {[1, 2, 3].map(i => (
          <div key={i} className="bg-surface-container-lowest rounded-2xl h-20 border border-outline-variant/10" />
        ))}
      </div>
    </div>
  );
      ...
      <div className="flex items-end justify-between mb-12">
        ...
        {!showForm && (
          <button
            ...
            className="flex items-center gap-2 bg-primary text-on-primary px-8 py-4 rounded-full font-serif font-medium hover:bg-[#3d4d41] transition-all disabled:opacity-40 shrink-0"
          >
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-outline-variant bg-surface-container-low py-32 text-center">
          ...
          {pets.length > 0 ? (
            <button
              onClick={() => setShowForm(true)}
              className="bg-primary text-on-primary px-8 py-4 rounded-full font-serif font-medium hover:bg-[#3d4d41] transition-all"
            >
              ...
              <div className="relative z-10">
                ...
                {new Date(featured.open_at) <= new Date() && (
                  <button
                    ...
                    disabled={isPending}
                    className="mt-8 flex items-center gap-2 bg-primary text-on-primary px-8 py-3 rounded-full font-serif font-medium hover:bg-[#3d4d41] transition-all"
                  >
