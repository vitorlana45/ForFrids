...
export default function TributeForm({ petId, petName, memorialSlug, onClose }: Props) {
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
        ...
        <h3 className="font-serif text-2xl text-on-surface">Mensagem enviada</h3>
        <p className="text-on-surface-variant max-w-xs">
          Sua homenagem a {petName} foi registrada com carinho.
        </p>
        <button
          onClick={onClose}
          className="mt-4 px-8 py-3 bg-primary text-on-primary rounded-full font-serif font-medium hover:bg-[#3d4d41] transition-all"
        >
          Fechar
        </button>
      </div>
